...
async function postImage(client) {
  const file = await axios({ method: 'get', url: transcript('files.cave-image'), responseType: 'stream' })
...
async function postAudio(client) {
  const file = await axios({ method: 'get', url: transcript('files.cave-audio'), responseType: 'stream' })
  console.log({channel: transcript('channels.cave')})
  const response = await client.files.upload({
    channels: transcript('channels.cave'),
    file: file.data,
