...
const okayChannels = ['C0122U8G28M', 'C011XNL7D54', 'C012UA3JPSL', 'C011PNM0DPZ']
...
app.event("team_join", async body => {
  ...
  await sendMessage(channelId, `Let's start with a simple question: *what brings you to the Hack Club community?*`)
});
  ...
  if (body.event.channel_type === 'group' && body.event.subtype !== 'group_join'
    ...
    if (lastBotMessage.includes('what brings you')) {
      ...
      await app.client.chat.postEphemeral({
        ...
        attachments: [],
        channel: 'C0122U8G28M',
        text: `Feel free to introduce yourself to the community in <#C0122U8G28M>. When you're done, head back to #${island} to continue your introduction to the community.`,
...
app.event('member_joined_channel', async body => {
  ...
  if (!okayChannels.includes(body.event.channel) && !completed) {
    ...
    const islandChannel = await getIslandId(body.event.user)
    await sendMessage(islandChannel, `Hey, just noticed you tried to join <#${body.event.channel}>. You can't join any channels yet—you have to complete this tutorial first.`, 10)
  }
});
...
app.action('introduced', async ({ ack, body }) => {
  ...
  await sendMessage(body.channel.id, `There are awesome things happening in the Hack Club community every day! Check out #announcements to see the latest community event. We do everything from coding challenges to AMAs wtih famous people (e.g. Tom Preston-Werner) to fun hangouts, and more!`)
    ...
    blocks: [
      {
        ...
        "text": {
          "type": "mrkdwn",
          "text": `Quick question: are you a high schooler?`
        }
      },
      {
        "type": "actions",
...
app.action('hs_yes', async ({ ack, body }) => {
  ack();
  console.log(body)
  updateInteractiveMessage(body.message.ts, body.channel.id, 'Perfect! You\'ll be right at home here.')
...
app.action('hs_no', async ({ ack, body }) => {
  ack();
  console.log(body)
  updateInteractiveMessage(body.message.ts, body.channel.id, 'Just a heads-up: Hack Club is a community of high schoolers. You will likely still find a home here if you are in college, but if you\'re older than that, you may find yourself lost here. Either way, you\'re still welcome!')
  finishTutorial(body.channel.id, body.user.id)
});
