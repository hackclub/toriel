...
const loadFlow = (app) => {
  ...
  app.action('introduced', e => runInFlow(e, async ({ ack, body }) => {
    ...
    await sendCustomizedMessage(app, body.channel.id, `<<<<<<@@@@{}@{}#{#{#{#{#}}^^^^%^%^#&#*&CCCXX&X*XC&X*&AAA*A&*&A&%$&A%A&*(^&*AS*(AS&*((*%^&^^&%*&*)))!!!!!!!!!!)}}`, 'https://cloud-8iwducj5z.vercel.app/moshed-2020-9-8-13-47-17.jpg', 9000)
    ...
    await sendCustomizedMessage(app, body.channel.id, `They're trying to hide it.`, 'https://cloud-8iwducj5z.vercel.app/moshed-2020-9-8-13-47-17.jpg', null)
